  ...
  return (
    <article class="container grid min-h-screen gap-8 pt-20">
      ...
      <p>
        ...
        only for my own usage. Why am i building my own, when there are many VPN
        service provider? It was because there is no transparency over how their
        server is managed. I often get bad internet access at the most needed of
        time. I know that most of the time it's my 4G Internet Service Provider
        that slows down, not the VPS server I am using. But is it true tho? Is
        it MY BAD 4G Internet access or is it just bad server management? And I
        can't even check because most of the time we only got notified ONLY when
        the server was down, I don't know how many users is connected to the
        same VPS as mine. So because of that, I plan to build VPN Service
        Provider who made sure that there is only smallest chance the users
        compete over the limited bandwidth of the server.
      </p>

      <p>
        Before building it, I try to list out features that I wanted to have. So
        the features I wrote was like this:
      </p>

      <ol class="list-decimal pl-4">
        <li>
          Users can top up their wallet through Payment Gateway, preferably via
        ...
        <li>
          Since we have a multiple location package, make it easier to do custom
          routing. Custom routing is an ability to filter some domain or
          application to use spesific server, or even block them.
        </li>

        <li>
          Infrastructure is managed using NixOS Flakes with Colmena tool to make
          it easy to add, manage, or update VPS. Automatically registering the
          server to DB and create DNS Records to Cloudflare API.
        </li>

        <li>
          Make the dashboard accessible from Telegram using Telegram Mini Apps,
          to make it convenient for Telegram users.
        </li>
      </ol>

      <p>
        So that's the feature I needed to make it possible. For the tech stack I
        ended up using was like these:
      </p>
      ...
      <p>
        ...
        can check video below explaining how to use SpeedVPN. You can also check
        out{" "}
        <a
